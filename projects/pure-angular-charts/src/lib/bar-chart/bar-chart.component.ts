  ...
  ngOnInit() {
    ...
    for (let i = 0; i <= this.pXaxis.ticks.count; i++) {
      xlabels.push({
        ...
        px: this.pYaxis.paddingLeft + (i * pPerUnitWidth) + (this.options.bar.width / 2),
        py: this.height
      });
    }
    ...
    this.pData = this.data.map((series, si) => {
      return {
        name: this.options.series[si],
        data: series.map((item, indx) => {
          const distanceFromXAxis = (((this.pYaxis.min < 0) ? item.y : (item.y - this.pYaxis.min)) / pPerUnitY) * pPerUnitHeight;
          // tslint:disable-next-line:max-line-length
          const distanceFromYAxis = (((this.pXaxis.min < 0) ? item.x : (item.x - this.pXaxis.min)) / pPerUnitX) * pPerUnitWidth + (this.options.bar.width / 2);
          return {
            x: item.x,
            y: item.y,
